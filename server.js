...
// Optymalizacja CORS - ograniczenie dostępu tylko dla Farcade
app.use(cors({ origin: "https://app.farcade.ai" }));
...
function getRandomSymbol(scatterCount) {
  ...
  let low = 0;
  let high = symbolWeights.length - 1;
...
function generateWildExpandMultiplier() {
  ...
  if (rand < 0.5) return 4;   // 50% szans na 4x
  if (rand < 0.8) return 10;  // 30% szans na 10x
  if (rand < 0.95) return 20; // 15% szans na 20x
  return 50;                  // 5% szans na 50x
}
...
app.post("/spin", (req, res) => {
  ...
  const rtpThreshold = 0.5;
  if (globalTurnover > 1000) {
    const currentRTP = globalTurnover > 0 ? (globalWins / globalTurnover) * 100 : 0;
    if (currentRTP < desiredRTP - rtpThreshold) {
      symbols.find((s) => s.id === "wild").weight *= adjustmentFactor;
    } else if (currentRTP > desiredRTP + rtpThreshold) {
      symbols.find((s) => s.id === "wild").weight /= adjustmentFactor;
    }
  }
